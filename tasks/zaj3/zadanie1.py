...
def load_data(path):
    ...
    with open(path, 'r') as file:
        readFile = csv.reader(file, dialect=csv.unix_dialect)
        for line in readFile:
            listaA.append(line[0])
            listaB.append(line[1])
            #print(lista)
        krotka = (listaA, listaB)
    #print(krotka)
    return krotka
...
def suggester(input, data):
    ...
    for linia in data[0]:
        if linia.startswith(input):
            sumowanie = sumowanie + int(data[1][iterator])
            slownik[linia[len(input)]] +=  int(data[1][iterator])
            #print(slownik)
        iterator = iterator + 1
    
    lista=[]
    for cokolwiek in slownik:
        slownik[cokolwiek] = slownik[cokolwiek] / sumowanie
        krotka = cokolwiek
        krotka1 = slownik[cokolwiek]
        lista.append((krotka, krotka1))
